...
//							end
			...
			fifo_to_axis #(CONTROL_DATA_SIZE, PIPELINE_DEPTH) control_signal_interface (.reset(reset), .clock(clock), .fifo_read_enable(fifo_read_enable), .fifo_empty(fifo_control_empty), .fifo_full(fifo_control_full), .fifo_data_out(fifo_control_out), .fifo_data_valid(fifo_control_valid), .tready_in(tready_in), .tvalid_out(control_valid_reg), .tdata_out(control_data_reg), .tlast_out(control_last_reg));
					...
					case (header_state)
						...
						begin
							...
							if (tvalid_in) begin
								...
								if (tlast_in) begin
									fifo_write_enable	<= tvalid_in;
									fifo_data_in		<= {{(BITS_PER_BEAT-HEADER_SIZE){1'b0}}, tdata_in[BITS_PER_BEAT-1:HEADER_SIZE]};
									fifo_control_in		<= {{(BYTES_PER_BEAT-BYTES_PER_HEADER){1'b0}}, tkeep_in[BITS_PER_BEAT/8-1:BYTES_PER_HEADER]};
									ipg_counter			<= 0;
									header_state		<= WAIT_FOR_END_OF_DATA;
								end
								else begin
						...
						begin
							if (tlast_in) begin
								if (tkeep_in[BYTES_PER_BEAT-1:BYTES_PER_HEADER] != {(BYTES_PER_BEAT-BYTES_PER_HEADER){1'b0}}) begin
									fifo_write_enable	<= tvalid_in;
...
//							header_data			<= {tdata_in[HEADER_SIZE-1:0], leftover_tdata};
						...
						begin
							...
							fifo_data_in		<= {{(BITS_PER_BEAT-HEADER_SIZE){1'b0}}, leftover_tdata};
